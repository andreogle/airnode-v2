...
module.exports.inheritance = function ({ item, build }) {
  ...
  return item.linearizedBaseContracts
    ...
    .filter((c, index) => c.name !== 'Context' || index === 0);
};
...
module.exports['inherited-functions'] = function ({ item }) {
  ...
  return inheritance.map((contract, index) => ({
    contract,
    functions: contract.functions.filter(f => !baseFunctions.has(f.id) && (f.name !== 'constructor' || index === 0)),
  }));
};
