...
function shouldBehaveLikeEIP6372(mode = 'blocknumber') {
  describe('should implement EIP6372', function () {
    ...
    it('CLOCK_MODE is correct', async function () {
      const parameters = new URLSearchParams(await this.mock.CLOCK_MODE());
      expect(parameters.get('mode')).to.be.equal(mode);
      expect(parameters.get('from')).to.be.equal(mode == 'blocknumber' ? 'default' : null);
    });
  });
}
