...
contract('ERC1155', function (accounts) {
  ...
  describe('internal functions', function () {
    ...
    const mintAmounts = [new BN(5000), new BN(10_000), new BN(42_195)];
      ...
      context('with minted batch of tokens', function () {
        ...
        it('credits the minted batch of tokens', async function () {
          const holderBatchBalances = await this.token.balanceOfBatch(
            Array.from({length: tokenBatchIds.length}).fill(tokenBatchHolder),
            tokenBatchIds,
          );

          for (const [index, holderBatchBalance] of holderBatchBalances.entries()) {
            expect(holderBatchBalance).to.be.bignumber.equal(mintAmounts[index]);
          }
        });
      });
    });
    ...
    describe('_burnBatch', function () {
      ...
      context('with minted-then-burnt tokens', function () {
        ...
        it('accounts for both minting and burning', async function () {
          ...
          for (const [index, holderBatchBalance] of holderBatchBalances.entries()) {
            expect(holderBatchBalance).to.be.bignumber.equal(mintAmounts[index].sub(burnAmounts[index]));
          }
        });
      });
    });
