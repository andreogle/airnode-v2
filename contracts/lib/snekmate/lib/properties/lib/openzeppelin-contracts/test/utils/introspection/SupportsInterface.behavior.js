...
for (const k of Object.getOwnPropertyNames(INTERFACES)) {
  ...
  for (const functionName of INTERFACES[k]) {
    // the interface id of a single function is equivalent to its function signature
    FN_SIGNATURES[functionName] = makeInterfaceId.ERC165([functionName]);
  }
}
  ...
  describe('ERC165', function () {
    ...
    it('supportsInterface uses less than 30k gas', async function () {
      for (const k of interfaces) {
        ...
        expect(await this.contractUnderTest.supportsInterface.estimateGas(interfaceId)).to.be.lte(30_000);
      }
    });
    ...
    it('all interface functions are in ABI', async function () {
      for (const k of interfaces) {
        ...
        for (const functionName of INTERFACES[k]) {
          const functionSig = FN_SIGNATURES[functionName];
          expect(this.contractUnderTest.abi.filter(function_ => function_.signature === functionSig).length).to.equal(1);
        }
      }
    });
  });
