...
const Error = ['None', 'RevertWithMessage', 'RevertWithoutMessage', 'Panic'].reduce(
  (accumulator, entry, index) => Object.assign({ [entry]: index }, accumulator),
  {},
);
  ...
  context('with minted tokens', function () {
    ...
    describe('transfers', function () {
      ...
      const shouldTransferTokensByUsers = function (transferFunction) {
        ...
        context('when sent to the owner', function () {
          ...
          it('keeps same tokens by index', async function () {
            if (!this.token.tokenOfOwnerByIndex) return;
            const tokensListed = await Promise.all([0, 1].map(index => this.token.tokenOfOwnerByIndex(owner, index)));
      ...
      describe('via transferFrom', function () {
        shouldTransferTokensByUsers(function (from, to, tokenId, options) {
          return this.token.transferFrom(from, to, tokenId, options);
        });
      });

      describe('via safeTransferFrom', function () {
        const safeTransferFromWithData = function (from, to, tokenId, options) {
          return this.token.methods['safeTransferFrom(address,address,uint256,bytes)'](from, to, tokenId, data, options);
        };

        const safeTransferFromWithoutData = function (from, to, tokenId, options) {
  ...
  context('with minted tokens', function () {
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns correct token IDs for target', async function () {
          ...
          const tokensListed = await Promise.all([0, 1].map(index => this.token.tokenOfOwnerByIndex(other, index)));
          expect(tokensListed.map(t => t.toNumber())).to.have.members([
            firstTokenId.toNumber(),
            secondTokenId.toNumber(),
          ]);
    ...
    describe('tokenByIndex', function () {
      it('returns all tokens', async function () {
        const tokensListed = await Promise.all([0, 1].map(index => this.token.tokenByIndex(index)));
        expect(tokensListed.map(t => t.toNumber())).to.have.members([
          firstTokenId.toNumber(),
          secondTokenId.toNumber(),
        ]);
      ...
      for (const tokenId of [firstTokenId, secondTokenId]) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
          ...
          const tokensListed = await Promise.all([0, 1, 2].map(index => this.token.tokenByIndex(index)));
