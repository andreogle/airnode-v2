    ...
    describe('safeBatchTransferFrom', function () {
      ...
      function batchTransferWasSuccessful({ operator, from, ids, values }) {
        it('debits transferred balances from sender', async function () {
          const newBalances = await this.token.balanceOfBatch(Array.from({length: ids.length}).fill(from), ids);
        ...
        it('credits transferred balances to receiver', async function () {
          const newBalances = await this.token.balanceOfBatch(Array.from({length: ids.length}).fill(this.toWhom), ids);
          for (const [index, newBalance] of newBalances.entries()) {
            expect(newBalance).to.be.a.bignumber.equal(values[index]);
          }
        });
