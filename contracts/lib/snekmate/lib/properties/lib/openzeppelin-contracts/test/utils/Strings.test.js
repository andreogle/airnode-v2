...
contract('Strings', function () {
  ...
  describe('toHexString', function () {
    ...
    it('converts a positive number', async function () {
      expect(await this.strings.methods['$toHexString(uint256)'](0x41_32)).to.equal('0x4132');
    });
  ...
  describe('toHexString fixed', function () {
    it('converts a positive number (long)', async function () {
      expect(await this.strings.methods['$toHexString(uint256,uint256)'](0x41_32, 32)).to.equal(
    ...
    it('converts a positive number (short)', async function () {
      await expectRevert(
        this.strings.methods['$toHexString(uint256,uint256)'](0x41_32, 1),
  ...
  describe('equal', function () {
    ...
    it('compares two different large strings', async function () {
      ...
      expect(await this.strings.methods['$equal(string,string)'](string1, string2)).to.equal(false);
    });

    it('compares two equal large strings', async function () {
      const string1 = 'a'.repeat(201);
      const string2 = 'a'.repeat(201);
      expect(await this.strings.methods['$equal(string,string)'](string1, string2)).to.equal(true);
    });
  });
});
