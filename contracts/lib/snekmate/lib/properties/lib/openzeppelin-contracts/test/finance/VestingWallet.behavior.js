...
function shouldBehaveLikeVesting(beneficiary) {
  it('check vesting schedule', async function () {
    const [vestedAmount, releasable, ...arguments_] = this.token
    ...
    for (const timestamp of this.schedule) {
      ...
      expect(await this.mock.methods[vestedAmount](...arguments_, timestamp)).to.be.bignumber.equal(vesting);

      expect(await this.mock.methods[releasable](...arguments_)).to.be.bignumber.equal(vesting);
    }
  });

  it('execute vesting schedule', async function () {
    const [release, ...arguments_] = this.token ? ['release(address)', this.token.address] : ['release()'];

    let released = web3.utils.toBN(0);
    const before = await this.getBalance(beneficiary);

    {
      const receipt = await this.mock.methods[release](...arguments_);
