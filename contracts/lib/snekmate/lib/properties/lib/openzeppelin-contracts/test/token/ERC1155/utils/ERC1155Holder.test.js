...
contract('ERC1155Holder', function (accounts) {
  ...
  it('receives ERC1155 tokens from a single ID', async function () {
    ...
    for (let index = 1; index < multiTokenIds.length; index++) {
      expect(await this.multiToken.balanceOf(this.holder.address, multiTokenIds[index])).to.be.bignumber.equal(new BN(0));
    }
  });

  it('receives ERC1155 tokens from a multiple IDs', async function () {
    for (const multiTokenId of multiTokenIds) {
      expect(await this.multiToken.balanceOf(this.holder.address, multiTokenId)).to.be.bignumber.equal(new BN(0));
    }
    ...
    for (const [index, multiTokenId] of multiTokenIds.entries()) {
      expect(await this.multiToken.balanceOf(this.holder.address, multiTokenId)).to.be.bignumber.equal(
        multiTokenAmounts[index],
      );
    }
  });
});
