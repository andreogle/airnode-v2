...
contract('SafeMath', function () {
  ...
  async function testCommutative(function_, lhs, rhs, expected, ...extra) {
    expect(await function_(lhs, rhs, ...extra)).to.be.bignumber.equal(expected);
    expect(await function_(rhs, lhs, ...extra)).to.be.bignumber.equal(expected);
  }

  async function testFailsCommutative(function_, lhs, rhs, reason, ...extra) {
    if (reason === undefined) {
      await expectRevert.unspecified(function_(lhs, rhs, ...extra));
      await expectRevert.unspecified(function_(rhs, lhs, ...extra));
    } else {
      await expectRevert(function_(lhs, rhs, ...extra), reason);
      await expectRevert(function_(rhs, lhs, ...extra), reason);
    }
  }

  async function testCommutativeIterable(function_, lhs, rhs, expected, ...extra) {
    expectStruct(await function_(lhs, rhs, ...extra), expected);
    expectStruct(await function_(rhs, lhs, ...extra), expected);
  }
