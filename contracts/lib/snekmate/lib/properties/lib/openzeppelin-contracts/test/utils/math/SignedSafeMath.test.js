...
contract('SignedSafeMath', function () {
  ...
  async function testCommutative(function_, lhs, rhs, expected) {
    expect(await function_(lhs, rhs)).to.be.bignumber.equal(expected);
    expect(await function_(rhs, lhs)).to.be.bignumber.equal(expected);
  }

  async function testFailsCommutative(function_, lhs, rhs) {
    await expectRevert.unspecified(function_(lhs, rhs));
    await expectRevert.unspecified(function_(rhs, lhs));
  }
