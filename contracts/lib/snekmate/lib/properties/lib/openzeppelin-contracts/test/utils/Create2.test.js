...
contract('Create2', function (accounts) {
  ...
  const encodedParameters = web3.eth.abi.encodeParameters(['address', 'uint64', 'uint64'], [other, 0, 0]).slice(2);

  const constructorByteCode = `${VestingWallet.bytecode}${encodedParameters}`;
