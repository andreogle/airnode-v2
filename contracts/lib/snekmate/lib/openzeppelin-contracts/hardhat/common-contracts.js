...
const INSTANCES = {
  ...
  entrypoint: {
    v07: {
      ...
      abi: JSON.parse(fs.readFileSync(path.resolve(__dirname, '../test/bin/EntryPoint070.abi'), 'utf8')),
      bytecode: fs.readFileSync(path.resolve(__dirname, '../test/bin/EntryPoint070.bytecode'), 'hex'),
    },
    v08: {
      address: '0x4337084D9E255Ff0702461CF8895CE9E3b5Ff108',
      abi: JSON.parse(fs.readFileSync(path.resolve(__dirname, '../test/bin/EntryPoint080.abi'), 'utf8')),
  ...
  senderCreator: {
    v07: {
      ...
      abi: JSON.parse(fs.readFileSync(path.resolve(__dirname, '../test/bin/SenderCreator070.abi'), 'utf8')),
    ...
    v08: {
      ...
      abi: JSON.parse(fs.readFileSync(path.resolve(__dirname, '../test/bin/SenderCreator080.abi'), 'utf8')),
...
task(TASK_TEST_SETUP_TEST_ENVIRONMENT).setAction((_, environment, runSuper) =>
  runSuper().then(() => setup(INSTANCES, environment.ethers).then(result => Object.assign(environment, result))),
);
