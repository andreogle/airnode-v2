...
// helpers
function _bound${capitalize(options.keyTypeName)}(${options.keyTypeName} x, ${options.keyTypeName} min, ${
  options.keyTypeName
} max) internal pure returns (${options.keyTypeName}) {
    return ${
      options.keyTypeName === 'uint256'
        ? 'bound(x, min, max)'
        : `SafeCast.to${capitalize(options.keyTypeName)}(bound(uint256(x), uint256(min), uint256(max)))`
    };
}

function _prepareKeys(${options.keyTypeName}[] memory keys, ${options.keyTypeName} maxSpread) internal pure {
...
function _assertLatestCheckpoint(bool exist, ${options.keyTypeName} key, ${options.valueTypeName} value) internal view {
...
// tests
function testPush(${options.keyTypeName}[] memory keys, ${options.valueTypeName}[] memory values, ${
  options.keyTypeName
} pastKey) public {
...
function testLookup(${options.keyTypeName}[] memory keys, ${options.valueTypeName}[] memory values, ${
  options.keyTypeName
} lookup) public {
    vm.assume(values.length > 0 && values.length <= keys.length);
    _prepareKeys(keys, _KEY_MAX_GAP);
...
module.exports = format(
  header,
  ...OPTS.flatMap(options => [
    `contract Checkpoints${options.historyTypeName}Test is Test {`,
    [template(options).trimEnd()],
    '}',
    '',
  ]),
);
