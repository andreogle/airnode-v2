...
) internal returns (${options.valueTypeName} oldValue, ${options.valueTypeName} newValue) {
 ...
 */
function latestCheckpoint(${options.historyTypeName} storage self) internal view returns (bool exists, ${options.keyTypeName} ${options.keyFieldName}, ${options.valueTypeName} ${options.valueFieldName}) {
    uint256 pos = self.${options.checkpointFieldName}.length;
    if (pos == 0) {
        return (false, 0, 0);
    } else {
...
function _insert(
    ...
    ${options.keyTypeName} key,
    ${options.valueTypeName} value
) private returns (${options.valueTypeName} oldValue, ${options.valueTypeName} newValue) {
    ...
    if (pos > 0) {
        ...
        if (lastKey == key) {
            last.${options.valueFieldName} = value;
        } else {
...
function _unsafeAccess(
    ${options.checkpointTypeName}[] storage self,
    uint256 pos
) private pure returns (${options.checkpointTypeName} storage result) {
    assembly {
        mstore(0, self.slot)
        result.slot := add(keccak256(0, 0x20), ${options.checkpointSize === 1 ? 'pos' : `mul(pos, ${options.checkpointSize})`})
    }
}
`;
...
module.exports = format(
  ...
  format(
    [errors].concat(
      OPTS.map(options => template(options)),
    ),
  ).trimEnd(),
  '}',
);
