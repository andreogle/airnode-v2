...
const formatArgumentsSingleLine = (...arguments_) => arguments_.filter(Boolean).join(', ');
const formatArgumentsMultiline = (...arguments_) => `\n${  format(arguments_.filter(Boolean).join(',\0').split('\0'))}`;
 ...
 */
function verify${suffix}(${(hash ? formatArgumentsMultiline : formatArgumentsSingleLine)(
 ...
 * This version handles proofs in ${location} with ${hash ? 'a custom' : 'the default'} hashing function.
 */
function processProof${suffix}(${(hash ? formatArgumentsMultiline : formatArgumentsSingleLine)(
  `bytes32[] ${location} proof`,
  'bytes32 leaf',
  hash && `function(bytes32, bytes32) view returns (bytes32) ${hash}`,
 ...
 */
function multiProofVerify${suffix}(${formatArgumentsMultiline(
  `bytes32[] ${location} proof`,
 ...
 */
function processMultiProof${suffix}(${formatArgumentsMultiline(
  `bytes32[] ${location} proof`,
  `bool[] ${location} proofFlags`,
  `bytes32[] memory leaves`,
  hash && `function(bytes32, bytes32) view returns (bytes32) ${hash}`,
...
module.exports = format(
  ...
  format(
    [errors].concat(
      OPTS.flatMap(options => templateProof(options)),
      OPTS.flatMap(options => templateMultiProof(options)),
    ),
  ).trimEnd(),
  '}',
);
