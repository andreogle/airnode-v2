...
const Report = {
  ...
  compare(update, reference, options = { hideEqual: true, strictTesting: false }) {
    if (JSON.stringify(update.options?.solcInfo) !== JSON.stringify(reference.options?.solcInfo)) {
    ...
    const referenceInfo = reference.info ?? reference.data;

    const deployments = updateInfo.deployments
      .map(contract =>
        Object.assign(contract, { previousVersion: referenceInfo.deployments.find(({ name }) => name === contract.name) }),
      )
    ...
    const methods = Object.keys(updateInfo.methods)
      .filter(key => referenceInfo.methods[key])
      .filter(key => updateInfo.methods[key].numberOfCalls > 0)
      .filter(
        key => !options.strictTesting || updateInfo.methods[key].numberOfCalls === referenceInfo.methods[key].numberOfCalls,
      )
      .map(key => ({
        contract: referenceInfo.methods[key].contract,
        method: referenceInfo.methods[key].fnSig,
        min: variation(...[updateInfo, referenceInfo].map(x => Math.min(...x.methods[key].gasData)), BASE_TX_COST),
        max: variation(...[updateInfo, referenceInfo].map(x => Math.max(...x.methods[key].gasData)), BASE_TX_COST),
        avg: variation(...[updateInfo, referenceInfo].map(x => Math.round(average(...x.methods[key].gasData))), BASE_TX_COST),
      }))
      .sort((a, b) => `${a.contract}:${a.method}`.localeCompare(`${b.contract}:${b.method}`));
