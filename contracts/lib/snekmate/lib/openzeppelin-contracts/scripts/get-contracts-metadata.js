...
module.exports = function (
    ...
    artifacts.flatMap(artifact => {
      ...
      return Object.keys(solcOutput.contracts)
        ...
        .map(source => [
          source,
          {
            pragma: [...findAll('PragmaDirective', solcOutput.sources[source].ast)]
              .find(({ literals }) => literals.at(0) == 'solidity')
              .literals.slice(1)
              .join(''),
            sources: [...findAll('ImportDirective', solcOutput.sources[source].ast)].map(
              ({ absolutePath }) => absolutePath,
            ),
            interface: [...findAll('ContractDefinition', solcOutput.sources[source].ast)].every(
  ...
  for (const file of glob
    ...
    .filter(file => !match.any(file, skipPatterns) && !Object.hasOwn(metadata, file))) {
      ...
      metadata[file] = {
        pragma: fs.readFileSync(file, 'utf8').match(/pragma solidity (?<pragma>[<>=^]*[0-9]+\.[0-9]+\.[0-9]+);/)
          ?.groups.pragma,
        sources: entries
