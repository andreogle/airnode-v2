...
class GovernorHelper {
  ...
  async vote(vote = {}) {
    ...
    const arguments_ = [await this.id, vote.support]; // base

    if (vote.signature) {
      const sign = await this.forgeMessage(vote).then(message => vote.signature(this.governor, message));
      if (vote.params || vote.reason) {
        method = 'castVoteWithReasonAndParamsBySig';
        arguments_.push(vote.voter, vote.reason ?? '', vote.params ?? '0x', sign);
      } else {
        method = 'castVoteBySig';
        arguments_.push(vote.voter, sign);
      }
    } else if (vote.params) {
      method = 'castVoteWithReasonAndParams';
      arguments_.push(vote.reason ?? '', vote.params);
    } else if (vote.reason) {
      method = 'castVoteWithReason';
      arguments_.push(vote.reason);
    }

    return await this.governor[method](...arguments_);
  }

  async overrideVote(vote = {}) {
    let method = 'castOverrideVote';
    const arguments_ = [await this.id, vote.support];

    vote.reason = vote.reason ?? '';

    if (vote.signature) {
      const sign = await this.forgeMessage(vote).then(message => vote.signature(this.governor, { reason: '', ...message }));
      method = 'castOverrideVoteBySig';
      arguments_.push(vote.voter, vote.reason ?? '', sign);
    }
