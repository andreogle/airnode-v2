...
class ERC7739Signer extends ethers.AbstractSigner {
  ...
  async signMessage(message) {
    return this.#signer.signTypedData(this.#domain, { PersonalSign }, ERC4337Utilities.preparePersonalSign(message));
  }

  async signTypedData(domain, types, value) {
    const { allTypes, contentsTypeName, contentsDescr } = ERC4337Utilities.getContentsDetail(types);

    return Promise.resolve(
      this.#signer.signTypedData(domain, allTypes, ERC4337Utilities.prepareSignTypedData(value, this.#domain)),
    ).then(signature =>
      [...ethers, 
        signature,
...
const ERC4337Utilities = {
  preparePersonalSign(message) {
    return {
      prefixed: [...ethers, 
  ...
  prepareSignTypedData(contents, signerDomain) {
    return {
  ...
  getContentsDetail(contentsTypes, contentsTypeName = Object.keys(contentsTypes).at(0)) {
    // Examples values
    //
    // contentsTypeName         B
