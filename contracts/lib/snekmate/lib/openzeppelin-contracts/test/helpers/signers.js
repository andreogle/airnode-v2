...
class NonNativeSigner extends AbstractSigner {
  ...
  async signTypedData(
    ...
    const populated = await TypedDataEncoder.resolveNames(domain, types, value, async name => {
      assert(this.provider != undefined, 'cannot resolve ENS names without a provider', 'UNSUPPORTED_OPERATION', {
        operation: 'resolveName',
        info: { name },
      });
      const address = await this.provider.resolveName(name);
      assert(address != undefined, 'unconfigured ENS name', 'UNCONFIGURED_NAME', { value: name });
      return address;
    });
