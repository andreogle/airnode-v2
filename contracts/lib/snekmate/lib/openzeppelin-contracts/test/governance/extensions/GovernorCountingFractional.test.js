      ...
      describe('voting with a fraction of the weight', function () {
        it('twice', async function () {
          ...
          for (const votes of steps) {
            const parameters = ethers.solidityPacked(['uint128', 'uint128', 'uint128'], votes);
            await expect(
          ...
          expect(await this.mock.usedVotes(this.proposal.id, this.voter2)).to.equal(sum(...steps.flat()));
        });
        ...
        it('fractional then nominal', async function () {
          ...
          const parameters = ethers.solidityPacked(['uint128', 'uint128', 'uint128'], fractional);
          await expect(
            this.helper.connect(this.voter2).vote({
              support: VoteType.Parameters,
              reason: 'no particular reason',
