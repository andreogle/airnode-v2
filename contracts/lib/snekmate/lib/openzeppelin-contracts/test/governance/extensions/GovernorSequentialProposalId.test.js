  ...
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`using ${Token}`, function () {
      ...
      it('sequential proposal ids', async function () {
        for (const index of iterate.range(1, 10)) {
          this.proposal.description = `<proposal description #${index}>`;
          ...
          await expect(this.mock.latestProposalId()).to.eventually.equal(index - 1);
          ...
          await expect(this.mock.hashProposal(...this.proposal.shortProposal)).to.eventually.equal(this.proposal.hash);
          await expect(this.mock.getProposalId(...this.proposal.shortProposal)).to.eventually.equal(index);
          await expect(this.mock.latestProposalId()).to.eventually.equal(index);
        }
      });
      ...
      it('sequential proposal ids with offset start', async function () {
        ...
        for (const index of iterate.range(offset + 1, offset + 10)) {
          this.proposal.description = `<proposal description #${index}>`;
