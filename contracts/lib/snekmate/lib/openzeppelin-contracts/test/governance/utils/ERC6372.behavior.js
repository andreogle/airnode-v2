...
function shouldBehaveLikeERC6372(mode = 'blocknumber') {
  describe(`ERC-6372 behavior in ${mode} mode`, function () {
    ...
    it('should have the correct CLOCK_MODE parameters', async function () {
      const clockModeParameters = new URLSearchParams(await this.mock.CLOCK_MODE());
      const expectedFromValue = mode === 'blocknumber' ? 'default' : null;

      expect(clockModeParameters.get('mode')).to.equal(mode, `Expected mode to be ${mode}`);
      expect(clockModeParameters.get('from')).to.equal(expectedFromValue, `Expected 'from' to be ${expectedFromValue}`);
    });
  });
}
