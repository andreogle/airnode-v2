      ...
      describe('should revert', function () {
        ...
        describe('on vote by signature', function () {
          ...
          it('if signature does not match signer', async function () {
            ...
            function tamper(string_, index, mask) {
              const arrayString = ethers.getBytes(string_);
              arrayString[index] ^= mask;
              return ethers.hexlify(arrayString);
            }
            ...
            const voteParameters = {
              ...
              voter: this.userEOA.address,
              nonce,
              signature: (...arguments_) => signBallot(this.userEOA)(...arguments_).then(sig => tamper(sig, 42, 0xFF)),
            };

            await expect(this.helper.vote(voteParameters))
              .to.be.revertedWithCustomError(this.mock, 'GovernorInvalidSignature')
              .withArgs(voteParameters.voter);
          });
