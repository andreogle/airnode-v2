  ...
  describe('AccountERC7579', function () {
    ...
    describe('module installation', function () {
      ...
      for (const moduleTypeId of [
        ...
        const fullData = [...ethers, prefix, initData];
    ...
    describe('execution', function () {
      ...
      for (const [execFunction, mock] of [
        ['execute', 'mockFromEntrypoint'],
        ['executeFromExecutor', 'mockFromExecutor'],
      ]) {
        describe(`executing with ${execFunction}`, function () {
          it('should revert if the call type is not supported', async function () {
            await expect(
              this[mock][execFunction](encodeMode({ callType: CALL_TYPE_INVALID }), encodeSingle(this.other, 0, '0x')),
            )
          ...
          it('should revert if the caller is not authorized / installed', async function () {
            const error = execFunction == 'execute' ? 'AccountUnauthorized' : 'ERC7579UninstalledModule';
            const arguments_ = execFunction == 'execute' ? [this.other] : [MODULE_TYPE_EXECUTOR, this.other];

            await expect(
              this[mock]
                .connect(this.other)
                [execFunction](encodeMode({ callType: CALL_TYPE_CALL }), encodeSingle(this.other, 0, '0x')),
            )
          ...
          describe('single execution', function () {
            it('calls the target with value and args', async function () {
              ...
              const tx = this[mock][execFunction](encodeMode({ callType: CALL_TYPE_CALL }), data);
            ...
            it('reverts when target reverts in default ExecType', async function () {
              ...
              await expect(this[mock][execFunction](encodeMode({ callType: CALL_TYPE_CALL }), data)).to.be.revertedWith(
            ...
            it('emits ERC7579TryExecuteFail event when target reverts in try ExecType', async function () {
              ...
              await expect(this[mock][execFunction](encodeMode({ callType: CALL_TYPE_CALL, execType: EXEC_TYPE_TRY }), data))
          ...
          describe('batch execution', function () {
            it('calls the targets with value and args', async function () {
              ...
              const tx = this[mock][execFunction](encodeMode({ callType: CALL_TYPE_BATCH }), data);
              await expect(tx)
            ...
            it('reverts when any target reverts in default ExecType', async function () {
              ...
              await expect(this[mock][execFunction](encodeMode({ callType: CALL_TYPE_BATCH }), data)).to.be.revertedWith(
                'CallReceiverMock: reverting',
              );
            });
            ...
            it('emits ERC7579TryExecuteFail event when any target reverts in try ExecType', async function () {
              ...
              const tx = this[mock][execFunction](encodeMode({ callType: CALL_TYPE_BATCH, execType: EXEC_TYPE_TRY }), data);
          ...
          describe('delegate call execution', function () {
            it('delegate calls the target', async function () {
              ...
              await this[mock][execFunction](encodeMode({ callType: CALL_TYPE_DELEGATE }), data);
            ...
            it('reverts when target reverts in default ExecType', async function () {
              ...
              await expect(this[mock][execFunction](encodeMode({ callType: CALL_TYPE_DELEGATE }), data)).to.be.revertedWith(
                'CallReceiverMock: reverting',
              );
            });
            ...
            it('emits ERC7579TryExecuteFail event when target reverts in try ExecType', async function () {
              ...
              await expect(
                this[mock][execFunction](encodeMode({ callType: CALL_TYPE_DELEGATE, execType: EXEC_TYPE_TRY }), data),
              )
                .to.emit(this.mock, 'ERC7579TryExecuteFail')
                .withArgs(
                  CALL_TYPE_CALL,
          ...
          withHooks &&
            describe('with hook', function () {
              ...
              it(`should call the hook of the installed module when executing ${execFunction}`, async function () {
                const caller = execFunction === 'execute' ? entrypoint.v08 : this.modules[MODULE_TYPE_EXECUTOR];
                const value = 17;
                ...
                const precheckData = this[mock].interface.encodeFunctionData(execFunction, [mode, call]);

                const tx = this[mock][execFunction](mode, call, { value });
