...
const fixture = async () => {
  ...
  const utilities = await ethers.deployContract('$ERC7579Utils', { value: ethers.parseEther('1') });
  const utilitiesGlobal = await ethers.deployContract('$ERC7579UtilsGlobalMock');
  const target = await ethers.deployContract('CallReceiverMock');
  const anotherTarget = await ethers.deployContract('CallReceiverMock');
  return { utils: utilities, utilsGlobal: utilitiesGlobal, target, anotherTarget, sender };
};
...
describe('ERC7579Utils', function () {
  ...
  it('encodes batch', async function () {
    const entries = [
      [this.target, 0x1_23, '0x12345678'],
      [this.anotherTarget, 0x4_56, '0x12345678'],
    ];
  ...
  it('decodes batch', async function () {
    const entries = [
      [this.target.target, 0x1_23, '0x12345678'],
      [this.anotherTarget.target, 0x4_56, '0x12345678'],
    ];
