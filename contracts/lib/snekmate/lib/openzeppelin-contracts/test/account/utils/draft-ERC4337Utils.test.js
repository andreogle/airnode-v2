...
const fixture = async () => {
  ...
  const utilities = await ethers.deployContract('$ERC4337Utils');
  const SIG_VALIDATION_SUCCESS = await utilities.$SIG_VALIDATION_SUCCESS();
  const SIG_VALIDATION_FAILED = await utilities.$SIG_VALIDATION_FAILED();

  return { utils: utilities, authorizer, sender, factory, paymaster, SIG_VALIDATION_SUCCESS, SIG_VALIDATION_FAILED };
};
...
describe('ERC4337Utils', function () {
  ...
  describe('parseValidationData', function () {
    it('parses the validation data', async function () {
      ...
      const validUntil = 0x12_34_56_78n;
      const validAfter = 0x9A_BC_DE_F0n;
    ...
    it('returns an type(uint48).max if until is 0', async function () {
      const authorizer = this.authorizer;
      const validAfter = 0x12_34_56_78n;
  ...
  describe('combineValidationData', function () {
    const validUntil1 = 0x12_34_56_78n;
    const validAfter1 = 0x9A_BC_DE_F0n;
    const validUntil2 = 0x87_65_43_21n;
    const validAfter2 = 0xAB_CD_EF_90n;
  ...
  describe('userOp values', function () {
    describe('intiCode', function () {
      beforeEach(async function () {
        this.userOp = new UserOperation({
          sender: this.sender,
          nonce: 1,
          verificationGas: 0x12_34_56_78n,
    ...
    it('returns verificationGasLimit', async function () {
      const userOp = new UserOperation({ sender: this.sender, nonce: 1, verificationGas: 0x12_34_56_78n });
    ...
    it('returns callGasLimit', async function () {
      const userOp = new UserOperation({ sender: this.sender, nonce: 1, callGas: 0x12_34_56_78n });
    ...
    it('returns maxPriorityFeePerGas', async function () {
      const userOp = new UserOperation({ sender: this.sender, nonce: 1, maxPriorityFee: 0x12_34_56_78n });
    ...
    it('returns maxFeePerGas', async function () {
      const userOp = new UserOperation({ sender: this.sender, nonce: 1, maxFeePerGas: 0x12_34_56_78n });
    ...
    it('returns gasPrice', async function () {
      const userOp = new UserOperation({
        sender: this.sender,
        nonce: 1,
        maxPriorityFee: 0x12_34_56_78n,
        maxFeePerGas: 0x87_65_43_21n,
      });
    ...
    describe('paymasterAndData', function () {
      beforeEach(async function () {
        this.userOp = new UserOperation({
          ...
          paymasterVerificationGasLimit: 0x12_34_56_78n,
          paymasterPostOpGasLimit: 0x87_65_43_21n,
