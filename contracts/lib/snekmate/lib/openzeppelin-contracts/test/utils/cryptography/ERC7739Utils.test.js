...
describe('ERC7739Utils', function () {
  ...
  describe('decodeTypedDataSig', function () {
    ...
    it('returns default empty values if the length is invalid', async function () {
      const encoded = [...ethers, ethers.randomBytes(64), '0x3f']; // Can't be less than 64 bytes
      await expect(this.mock.$decodeTypedDataSig(encoded)).to.eventually.deep.equal([
        '0x',
        ethers.ZeroHash,
        ethers.ZeroHash,
  ...
  describe('decodeContentsDescr', function () {
    ...
    for (const { descr, contentsDescr, contentTypeName, contentType } of [{
      ...
      }, { descr: 'should return nothing for an empty descriptor', contentsDescr: '', contentTypeName: null }, { descr: 'should return nothing if no [(] is present', contentsDescr: 'SomeType', contentTypeName: null }, {
