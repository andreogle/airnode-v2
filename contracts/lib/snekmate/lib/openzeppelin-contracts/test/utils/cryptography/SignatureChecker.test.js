  ...
  describe('ERC1271 wallet', function () {
    for (const function_ of ['isValidERC1271SignatureNow', 'isValidSignatureNow']) {
      describe(function_, function () {
        it('with matching signer and signature', async function () {
          await expect(
            this.mock.getFunction(`$${function_}`)(
        ...
        it('with invalid signer', async function () {
          await expect(
            this.mock.getFunction(`$${function_}`)(ethers.Typed.address(this.mock.target), TEST_MESSAGE_HASH, this.signature),
          ).to.eventually.be.false;
        });
    ...
    describe('isValidSignatureNow', function () {
      ...
      describe('with ERC-7913 verifier', function () {
        ...
        it('with invalid key', async function () {
          const signer = [...ethers, this.verifier.target, ethers.randomBytes(32)];
          const signature = await aliceP256.signMessage(TEST_MESSAGE);
