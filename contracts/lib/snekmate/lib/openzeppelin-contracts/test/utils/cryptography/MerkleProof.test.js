...
const toLeaves = (string_, separator = '') => string_.split(separator).map(e => ethers.keccak256(ethers.toUtf8Bytes(e)));
  ...
  for (const { title, contractName, nodeHash } of [
    ...
    describe(title, function () {
      ...
      before(async function () {
        ...
        this.makeTree = string_ => SimpleMerkleTree.of(toLeaves(string_), { nodeHash });
      });
