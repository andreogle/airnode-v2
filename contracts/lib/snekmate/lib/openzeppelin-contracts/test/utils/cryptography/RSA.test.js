...
describe('RSA', function () {
  ...
  describe('SigVer15_186-3.rsp tests', function () {
    for (const test of parse('SigVer15_186-3.rsp')) {
      ...
      if (test.SHAAlg === 'SHA256' && length >= 0x1_00) {
        ...
        it(`signature length ${length} ${test.extra} ${result ? 'works' : 'fails'}`, async function () {
          ...
          const sig = `0x${  test.S}`;
          const exp = `0x${  test.e}`;
          const module_ = `0x${  test.n}`;

          expect(await this.mock.$pkcs1Sha256(bytes32(ethers.sha256(data)), sig, exp, module_)).to.equal(result);
          expect(await this.mock.$pkcs1Sha256(bytes(data), sig, exp, module_)).to.equal(result);
        });
      }
    }
  });
