...
const N = 0xFF_FF_FF_FF_00_00_00_00_FF_FF_FF_FF_FF_FF_FF_FF_BC_E6_FA_AD_A7_17_9E_84_F3_B9_CA_C2_FC_63_25_51n;
    ...
    for (const { key, tests } of require('./ecdsa_secp256r1_sha256_p1363_test.json').testGroups) {
      ...
      for (const { tcId, comment, msg, sig, result } of tests) {
        ...
        it(`${tcId}: ${comment}`, async function () {
          ...
          let [r, s] = Array.from({length: 2})
            .fill()
            .map((_, index) => ethers.toBigInt(`0x${  sig.substring(64 * index, 64 * (index + 1))}`));
