    ...
    it('insert, pop and replace', async function () {
      ...
      for (const { op, value } of [
        ...
        switch (op) {
          ...
          case 'pop': {
            await (heap.length === 0 ? expect(this.mock.$pop(0)).to.be.revertedWithPanic(PANIC_CODES.POP_ON_EMPTY_ARRAY) : expect(this.mock.$pop(0)).to.emit(this.mock, 'return$pop').withArgs(heap.shift()));
            break;
          }
          case 'replace': {
            if (heap.length === 0) {
