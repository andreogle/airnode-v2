...
describe('MerkleTree', function () {
  ...
  describe('push', function () {
    it('pushing correctly updates the tree', async function () {
      ...
      // for each leaf slot
      for (const index in range(2 ** DEPTH)) {
        // generate random leaf
        leaves.push(generators.bytes32());

        // rebuild tree.
        const tree = makeTree(leaves);
        const hash = tree.leafHash(tree.at(index));

        // push value to tree
        await expect(this.mock.push(hash)).to.emit(this.mock, 'LeafInserted').withArgs(hash, index, tree.root);

        // check tree
        await expect(this.mock.root()).to.eventually.equal(tree.root);
        await expect(this.mock.nextLeafIndex()).to.eventually.equal(BigInt(index) + 1n);
      }
    });
  ...
  describe('update', function () {
    for (const { leafCount, leafIndex } of range(2 ** DEPTH + 1).flatMap(leafCount =>
      ...
      {it(`updating a leaf correctly updates the tree (leaf #${leafIndex + 1}/${leafCount})`, async function () {
        ...
        for (const index in leaves) {
          await this.mock.push(oldTree.leafHash(oldTree.at(index)));
        }
        ...
        for (const index of range(leafCount, 2 ** DEPTH)) {
          // push new value and rebuild tree
          leaves.push(generators.bytes32());
          const nextTree = makeTree(leaves);

          // push and verify root
          await this.mock.push(nextTree.leafHash(nextTree.at(index)));
    ...
    it('replacing a leaf using an invalid proof reverts', async function () {
      ...
      // fill tree and verify root
      for (const index in leaves) {
        await this.mock.push(tree.leafHash(tree.at(index)));
      }
      await expect(this.mock.root()).to.eventually.equal(tree.root);
