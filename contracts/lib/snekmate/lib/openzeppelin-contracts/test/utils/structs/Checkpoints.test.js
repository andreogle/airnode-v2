  ...
  for (const opt of OPTS) {
    describe(opt.historyTypeName, function () {
      const fixture = async () => {
        ...
        const methods = {
          at: (...arguments_) => mock.getFunction(`$at_Checkpoints_${opt.historyTypeName}`)(0, ...arguments_),
          latest: (...arguments_) => mock.getFunction(`$latest_Checkpoints_${opt.historyTypeName}`)(0, ...arguments_),
          latestCheckpoint: (...arguments_) =>
            mock.getFunction(`$latestCheckpoint_Checkpoints_${opt.historyTypeName}`)(0, ...arguments_),
          length: (...arguments_) => mock.getFunction(`$length_Checkpoints_${opt.historyTypeName}`)(0, ...arguments_),
          push: (...arguments_) => mock.getFunction(`$push(uint256,${opt.keyTypeName},${opt.valueTypeName})`)(0, ...arguments_),
          lowerLookup: (...arguments_) => mock.getFunction(`$lowerLookup(uint256,${opt.keyTypeName})`)(0, ...arguments_),
          upperLookup: (...arguments_) => mock.getFunction(`$upperLookup(uint256,${opt.keyTypeName})`)(0, ...arguments_),
          upperLookupRecent: (...arguments_) =>
            mock.getFunction(`$upperLookupRecent(uint256,${opt.keyTypeName})`)(0, ...arguments_),
        };
      ...
      describe('with checkpoints', function () {
        ...
        it('lower lookup', async function () {
          for (let index = 0; index < 14; ++index) {
            const value = this.checkpoints.find(x => index <= x.key)?.value || 0n;

            expect(await this.methods.lowerLookup(index)).to.equal(value);
          }
        });

        it('upper lookup & upperLookupRecent', async function () {
          for (let index = 0; index < 14; ++index) {
            const value = this.checkpoints.findLast(x => index >= x.key)?.value || 0n;

            expect(await this.methods.upperLookup(index)).to.equal(value);
            expect(await this.methods.upperLookupRecent(index)).to.equal(value);
          }
        });
        ...
        it('upperLookupRecent with more than 5 checkpoints', async function () {
          ...
          for (let index = 0; index < 25; ++index) {
            const value = allCheckpoints.findLast(x => index >= x.key)?.value || 0n;
            expect(await this.methods.upperLookup(index)).to.equal(value);
            expect(await this.methods.upperLookupRecent(index)).to.equal(value);
          }
        });
