...
function shouldBehaveLikeRegularContext() {
  ...
  describe('msgData', function () {
    ...
    it('returns the transaction data when called from an EOA', async function () {
      const callData = this.context.interface.encodeFunctionData('msgData', arguments_);

      await expect(this.context.msgData(...arguments_))
        .to.emit(this.context, 'Data')
        .withArgs(callData, ...arguments_);
    });

    it('returns the transaction sender when from another contract', async function () {
      const callData = this.context.interface.encodeFunctionData('msgData', arguments_);

      await expect(this.contextHelper.callData(this.context, ...arguments_))
        .to.emit(this.context, 'Data')
        .withArgs(callData, ...arguments_);
    });
  });
