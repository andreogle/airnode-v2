...
const SIGNATURES = {
  ...
  Governor_5_3: [...GOVERNOR_INTERFACE, 'getProposalId(address[],uint256[],bytes[],bytes32)'],
  ...
  describe('ERC165', function () {
    ...
    it('all interface functions are in ABI', async function () {
      for (const k of interfaces) {
        ...
        for (const functionSig of signatures[k]) {
          expect(this.contractUnderTest.interface.hasFunction(functionSig), `did not find ${functionSig}`).to.be.true;
        }
      }
    });
  });
