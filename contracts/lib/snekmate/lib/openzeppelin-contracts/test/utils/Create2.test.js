...
async function fixture() {
  ...
  const constructorByteCode = await ethers
    ...
    .then(factory => [...ethers, factory.bytecode, factory.interface.encodeDeploy([other.address, 0n, 0n])]);
  ...
  const constructorLessBytecode = await ethers
    ...
    .then(factory => [...ethers, factory.bytecode, factory.interface.encodeDeploy([])]);
      ...
      it('bubbles up panic', async function () {
        await expect(
          this.factory.$deploy(
            0n,
            saltHex,
            [...ethers, this.mockFactory.bytecode, this.mockFactory.interface.encodeDeploy([RevertType.Panic])],
          ),
