...
describe('ERC4626', function () {
  ...
  it('multiple mint, deposit, redeem & withdrawal', async function () {
    ...
    expect(await vault.totalAssets()).to.equal(11_000n);
    ...
    expect(await vault.totalAssets()).to.equal(14_000n); // used to be 14001
    ...
    expect(await vault.convertToAssets(await vault.balanceOf(bruce))).to.equal(10_928n); // used to be 10929
    expect(await vault.convertToShares(await token.balanceOf(vault))).to.equal(9333n);
    expect(await vault.totalSupply()).to.equal(9333n);
    expect(await vault.totalAssets()).to.equal(17_000n); // used to be 17001
    ...
    expect(await vault.convertToAssets(await vault.balanceOf(bruce))).to.equal(10_929n);
    ...
    expect(await vault.totalAssets()).to.equal(14_573n);
    ...
    expect(await vault.totalAssets()).to.equal(11_644n);
