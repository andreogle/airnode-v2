...
task(TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOB_FOR_FILE, async (parameters, _, runSuper) => {
  const job = await runSuper(parameters);
  ...
  if (parameters.file.sourceName.startsWith('contracts/mocks/') && /\bunreachable\b/.test(parameters.file.sourceName)) {
...
task(TASK_COMPILE_SOLIDITY_COMPILE, async (parameters, _, runSuper) => {
  const marked = parameters.compilationJob.solidityConfig[marker];
  const result = await runSuper(parameters);
  if (marked) {
    result.output = {
      ...result.output,
