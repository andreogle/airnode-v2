...
internalTask(TASK_TEST_GET_TEST_FILES).setAction(async (arguments_, hre, runSuper) => {
  ...
  return (await runSuper(arguments_)).filter(file => hasProxies || !ignoredIfProxy.has(file));
});
