...
function match(entry, request) {
  const [requestSpec, requestContract] = request.split(':').reverse();
  return entry.spec == requestSpec && (!requestContract || entry.contract == requestContract);
}
...
async function runCertora(spec, contract, files, options = []) {
  const arguments_ = [...files, '--verify', `${contract}:certora/specs/${spec}.spec`, ...options];
  if (argv.verbose) {
    console.log('Running:', arguments_.join(' '));
  }
  const child = proc.spawn('certoraRun', arguments_);
  ...
  // write all details
  console.error(`+ certoraRun ${arguments_.join(' ')}\n${  await output}`);
}
