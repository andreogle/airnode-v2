...
describe('Strings', function () {
  ...
  describe('toString', function () {
    const values = [
      ...
      123_456_789_012_345_678_901_234_567_890_123_456_789_012_345_678_901_234_567_890n,
  ...
  describe('toHexString', function () {
    ...
    it('converts a positive number', async function () {
      expect(await this.mock.getFunction('$toHexString(uint256)')(0x41_32n)).to.equal('0x4132');
    });
  ...
  describe('toHexString fixed', function () {
    it('converts a positive number (long)', async function () {
      expect(await this.mock.getFunction('$toHexString(uint256,uint256)')(0x41_32n, 32n)).to.equal(
    ...
    it('converts a positive number (short)', async function () {
      const length = 1n;
      await expect(this.mock.getFunction('$toHexString(uint256,uint256)')(0x41_32n, length))
        .to.be.revertedWithCustomError(this.mock, `StringsInsufficientHexLength`)
        .withArgs(0x41_32, length);
    });
  ...
  describe('equal', function () {
    ...
    it('compares two different large strings', async function () {
      const string1 = 'a'.repeat(201);
      const string2 = `${'a'.repeat(200)  }b`;
      expect(await this.mock.$equal(string1, string2)).to.be.false;
    });

    it('compares two equal large strings', async function () {
      const string1 = 'a'.repeat(201);
      const string2 = 'a'.repeat(201);
      expect(await this.mock.$equal(string1, string2)).to.be.true;
    });
  });
});
