  ...
  describe('when receiving a relayed call', function () {
    describe('msgSender', function () {
      it('returns the relayed transaction original sender', async function () {
        ...
        request.signature = await this.sender.signTypedData(this.domain, this.types, request);

        expect(await this.forwarder.verify(request)).to.be.true;

        await expect(this.forwarder.execute(request)).to.emit(this.context, 'Sender').withArgs(this.sender);
      });
    ...
    describe('msgData', function () {
      it('returns the relayed transaction original data', async function () {
        const arguments_ = [42n, 'OpenZeppelin'];

        const nonce = await this.forwarder.nonces(this.sender);
        const data = this.context.interface.encodeFunctionData('msgData', arguments_);
        ...
        request.signature = this.sender.signTypedData(this.domain, this.types, request);

        expect(await this.forwarder.verify(request)).to.be.true;

        await expect(this.forwarder.execute(request))
          .to.emit(this.context, 'Data')
          .withArgs(data, ...arguments_);
      });
    });
  ...
  it('multicall poison attack', async function () {
    ...
    const data = this.context.interface.encodeFunctionData('multicall', [
      [
        ...
        [...ethers, this.context.interface.encodeFunctionData('msgSender'), this.other.address],
      ],
    ]);
