...
describe('MerkleTree', function () {
  ...
  describe('push', function () {
    it('tree is correctly updated', async function () {
      ...
      // for each leaf slot
      for (const index in leafs) {
        // generate random leaf and hash it
        const hashedLeaf = hashLeaf((leafs[index] = generators.bytes32()));

        // update leaf list and rebuild tree.
        const tree = makeTree(leafs);

        // push value to tree
        await expect(this.mock.push(hashedLeaf)).to.emit(this.mock, 'LeafInserted').withArgs(hashedLeaf, index, tree.root);

        // check tree
        expect(await this.mock.root()).to.equal(tree.root);
        expect(await this.mock.nextLeafIndex()).to.equal(BigInt(index) + 1n);
      }
    });
