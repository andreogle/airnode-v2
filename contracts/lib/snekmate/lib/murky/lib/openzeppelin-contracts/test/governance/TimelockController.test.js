...
const getAddress = object => object.address ?? object.target ?? object;
  ...
  describe('methods', function () {
    describe('operation hashing', function () {
      ...
      it('hashOperationBatch', async function () {
        this.operation = genOperationBatch(
          Array.from({length: 8}).fill('0x2d5f21620e56531c1d59c2df9b8e95d129571f71'),
          Array.from({length: 8}).fill('0x2b993cfce932ccee'),
          Array.from({length: 8}).fill('0xcf51966b'),
    ...
    describe('batch', function () {
      describe('schedule', function () {
        beforeEach(async function () {
          this.operation = genOperationBatch(
            Array.from({length: 8}).fill('0xEd912250835c812D4516BBD80BdaEA1bB63a293C'),
            Array.from({length: 8}).fill(0n),
            Array.from({length: 8}).fill('0x2fcb7a88'),
            ethers.ZeroHash,
        ...
        it('proposer can schedule', async function () {
          ...
          for (const index in this.operation.targets) {
            await expect(tx)
              .to.emit(this.mock, 'CallScheduled')
              .withArgs(
                this.operation.id,
                index,
                getAddress(this.operation.targets[index]),
                this.operation.values[index],
                this.operation.payloads[index],
                this.operation.predecessor,
                MINDELAY,
              )
              .to.emit(this.mock, 'CallSalt')
      ...
      describe('execute', function () {
        beforeEach(async function () {
          this.operation = genOperationBatch(
            Array.from({length: 8}).fill('0x76E53CcEb05131Ef5248553bEBDb8F70536830b1'),
            Array.from({length: 8}).fill(0n),
            Array.from({length: 8}).fill('0x58a60f63'),
            ethers.ZeroHash,
        ...
        describe('with scheduled operation', function () {
          ...
          describe('on time', function () {
            ...
            it('prevents reentrancy execution', async function () {
              ...
              for (const index in nonReentrantBatchOperation.targets) {
                await expect(tx)
                  .to.emit(this.mock, 'CallExecuted')
                  .withArgs(
                    nonReentrantBatchOperation.id,
                    index,
                    nonReentrantBatchOperation.targets[index],
                    nonReentrantBatchOperation.values[index],
                    nonReentrantBatchOperation.payloads[index],
                  );
              }
            });
          });
