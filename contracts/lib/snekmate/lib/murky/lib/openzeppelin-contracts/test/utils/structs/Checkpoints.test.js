  ...
  for (const length of VALUE_SIZES) {
    describe(`Trace${length}`, function () {
      const fixture = async () => {
        ...
        const methods = {
          at: (...arguments_) => mock.getFunction(`$at_Checkpoints_Trace${length}`)(0, ...arguments_),
          latest: (...arguments_) => mock.getFunction(`$latest_Checkpoints_Trace${length}`)(0, ...arguments_),
          latestCheckpoint: (...arguments_) => mock.getFunction(`$latestCheckpoint_Checkpoints_Trace${length}`)(0, ...arguments_),
          length: (...arguments_) => mock.getFunction(`$length_Checkpoints_Trace${length}`)(0, ...arguments_),
          push: (...arguments_) => mock.getFunction(`$push(uint256,uint${256 - length},uint${length})`)(0, ...arguments_),
          lowerLookup: (...arguments_) => mock.getFunction(`$lowerLookup(uint256,uint${256 - length})`)(0, ...arguments_),
          upperLookup: (...arguments_) => mock.getFunction(`$upperLookup(uint256,uint${256 - length})`)(0, ...arguments_),
          upperLookupRecent: (...arguments_) =>
            mock.getFunction(`$upperLookupRecent(uint256,uint${256 - length})`)(0, ...arguments_),
        };
