...
function shouldBehaveLikeERC721() {
  ...
  describe('with minted tokens', function () {
    ...
    describe('transfers', function () {
      ...
      const shouldTransferTokensByUsers = function (fragment, options = {}) {
        describe('when called by the owner', function () {
          beforeEach(async function () {
            this.tx = () =>
              this.token.connect(this.owner)[fragment](this.owner, this.to, tokenId, ...(options.extra ?? []));
          });
        ...
        describe('when called by the approved individual', function () {
          beforeEach(async function () {
            this.tx = () =>
              this.token.connect(this.approved)[fragment](this.owner, this.to, tokenId, ...(options.extra ?? []));
          });
          transferWasSuccessful();
        });
        ...
        describe('when called by the operator', function () {
          beforeEach(async function () {
            this.tx = () =>
              this.token.connect(this.operator)[fragment](this.owner, this.to, tokenId, ...(options.extra ?? []));
          });
          transferWasSuccessful();
        });
        ...
        describe('when sent to the owner', function () {
          beforeEach(async function () {
            this.tx = () =>
              this.token.connect(this.owner)[fragment](this.owner, this.owner, tokenId, ...(options.extra ?? []));
          });
          ...
          it('keeps same tokens by index', async function () {
            ...
            expect(await Promise.all([0n, 1n].map(index => this.token.tokenOfOwnerByIndex(this.owner, index)))).to.have.members(
        ...
        describe('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(
              this.token.connect(this.owner)[fragment](this.other, this.other, tokenId, ...(options.extra ?? [])),
            )
        ...
        describe('when the sender is not authorized for the token id', function () {
          if (options.unrestricted) {
            it('does not revert', async function () {
              await this.token.connect(this.other)[fragment](this.owner, this.other, tokenId, ...(options.extra ?? []));
            });
          } else {
            it('reverts', async function () {
              await expect(
                this.token.connect(this.other)[fragment](this.owner, this.other, tokenId, ...(options.extra ?? [])),
              )
        ...
        describe('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expect(
              this.token
                .connect(this.owner)
                [fragment](this.owner, this.other, nonExistentTokenId, ...(options.extra ?? [])),
            )
              .to.be.revertedWithCustomError(this.token, 'ERC721NonexistentToken')
        ...
        describe('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(
              this.token.connect(this.owner)[fragment](this.owner, ethers.ZeroAddress, tokenId, ...(options.extra ?? [])),
            )
      ...
      const shouldTransferSafely = function (fragment, data, options = {}) {
        // sanity
        ...
        describe('to a user account', function () {
          shouldTransferTokensByUsers(fragment, options);
        });
        ...
        describe('to a valid receiver contract', function () {
          ...
          it('calls onERC721Received', async function () {
            await expect(this.token.connect(this.owner)[fragment](this.owner, this.to, tokenId, ...(options.extra ?? [])))
            ...
            it('reverts', async function () {
              await expect(
                this.token
                  ...
                  [fragment](this.owner, this.to, nonExistentTokenId, ...(options.extra ?? [])),
              )
                .to.be.revertedWithCustomError(this.token, 'ERC721NonexistentToken')
                .withArgs(nonExistentTokenId);
            });
  ...
  describe('with minted tokens', function () {
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns correct token IDs for target', async function () {
          ...
          expect(await Promise.all([0n, 1n].map(index => this.token.tokenOfOwnerByIndex(this.other, index)))).to.have.members([
            firstTokenId,
            secondTokenId,
          ]);
        });
    ...
    describe('tokenByIndex', function () {
      it('returns all tokens', async function () {
        expect(await Promise.all([0n, 1n].map(index => this.token.tokenByIndex(index)))).to.have.members([
          firstTokenId,
          secondTokenId,
        ]);
      });
      ...
      for (const tokenId of [firstTokenId, secondTokenId]) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
          ...
          expect(await Promise.all([0n, 1n, 2n].map(index => this.token.tokenByIndex(index))))
