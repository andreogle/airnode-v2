module.exports = {
  ...
  range: (start, stop, step = 1) => {
    ...
    return start < stop ? Array.from({ length: Math.ceil((stop - start) / step) }, (_, index) => start + index * step) : [];
  },
  ...
  mapValues: (object, function_) => Object.fromEntries(Object.entries(object).map(([k, v]) => [k, function_(v)])),
};
