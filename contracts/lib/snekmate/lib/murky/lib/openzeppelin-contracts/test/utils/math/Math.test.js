...
const bytes = (value, width) => ethers.Typed.bytes(ethers.toBeHex(value, width));
const uint256 = value => ethers.Typed.uint256(value);
bytes.zero = '0x';
uint256.zero = 0n;

async function testCommutative(function_, lhs, rhs, expected, ...extra) {
  expect(await function_(lhs, rhs, ...extra)).to.deep.equal(expected);
  expect(await function_(rhs, lhs, ...extra)).to.deep.equal(expected);
}
...
describe('Math', function () {
  ...
  describe('invMod', function () {
    for (const factors of [
      ...
      const p = factors.reduce((accumulator, f) => accumulator * f, 1n);
  ...
  describe('sqrt', function () {
    it('rounds down', async function () {
      for (const rounding of RoundingDown) {
        ...
        expect(await this.mock.$sqrt(999_999n, rounding)).to.equal(999n);
        expect(await this.mock.$sqrt(1_000_000n, rounding)).to.equal(1000n);
        expect(await this.mock.$sqrt(1_000_001n, rounding)).to.equal(1000n);
        expect(await this.mock.$sqrt(1_002_000n, rounding)).to.equal(1000n);
        expect(await this.mock.$sqrt(1_002_001n, rounding)).to.equal(1001n);
        expect(await this.mock.$sqrt(ethers.MaxUint256, rounding)).to.equal(340_282_366_920_938_463_463_374_607_431_768_211_455n);
      }
    });
    ...
    it('rounds up', async function () {
      for (const rounding of RoundingUp) {
        ...
        expect(await this.mock.$sqrt(4n, rounding)).to.equal(2n);
        expect(await this.mock.$sqrt(144n, rounding)).to.equal(12n);
        expect(await this.mock.$sqrt(999_999n, rounding)).to.equal(1000n);
        expect(await this.mock.$sqrt(1_000_000n, rounding)).to.equal(1000n);
        expect(await this.mock.$sqrt(1_000_001n, rounding)).to.equal(1001n);
        expect(await this.mock.$sqrt(1_002_000n, rounding)).to.equal(1001n);
        expect(await this.mock.$sqrt(1_002_001n, rounding)).to.equal(1001n);
        expect(await this.mock.$sqrt(ethers.MaxUint256, rounding)).to.equal(340_282_366_920_938_463_463_374_607_431_768_211_456n);
      }
    });
  });
    ...
    describe('log256', function () {
      it('rounds down', async function () {
        for (const rounding of RoundingDown) {
          ...
          expect(await this.mock.$log256(65_535n, rounding)).to.equal(1n);
          expect(await this.mock.$log256(65_536n, rounding)).to.equal(2n);
          expect(await this.mock.$log256(65_537n, rounding)).to.equal(2n);
      ...
      it('rounds up', async function () {
        for (const rounding of RoundingUp) {
          ...
          expect(await this.mock.$log256(65_535n, rounding)).to.equal(2n);
          expect(await this.mock.$log256(65_536n, rounding)).to.equal(2n);
          expect(await this.mock.$log256(65_537n, rounding)).to.equal(3n);
