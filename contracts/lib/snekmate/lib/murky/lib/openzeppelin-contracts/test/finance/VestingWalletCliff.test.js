...
async function fixture() {
  ...
  const environment = await envSetup(mock, beneficiary, token);

  const schedule = Array.from({ length: 64 }, (_, index) => (BigInt(index) * duration) / 60n + start);
  const vestingFunction = timestamp => min(amount, timestamp < cliff ? 0n : (amount * (timestamp - start)) / duration);

  return { mock, duration, start, beneficiary, cliff, schedule, vestingFn: vestingFunction, env: environment };
}
