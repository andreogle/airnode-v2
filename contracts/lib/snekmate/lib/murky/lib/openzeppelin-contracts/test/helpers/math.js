...
const max = (...values) => values.slice(1).reduce((x, y) => (Math.max(x, y)), values.at(0));
const min = (...values) => values.slice(1).reduce((x, y) => (Math.min(x, y)), values.at(0));
const sum = (...values) => values.slice(1).reduce((x, y) => x + y, values.at(0));

// Computes modexp without BigInt overflow for large numbers
function moduleExp(b, e, m) {
  let result = 1n;
